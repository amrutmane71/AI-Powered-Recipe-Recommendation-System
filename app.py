...
# Train KNN Model
# Fetch more neighbors to allow for filtering
N_NEIGHBORS_TO_FETCH = 20
knn = NearestNeighbors(n_neighbors=N_NEIGHBORS_TO_FETCH, metric='euclidean')
...
def recommend_recipes(input_ingredients, selected_region):
    ...
    # Get the initial list of recommendations
    recommendations = data.iloc[indices[0]]

    # Filter by region if a specific one is selected
    if selected_region != "All":
        recommendations = recommendations[recommendations['Region'] == selected_region]

    # Return the top 5 results after filtering
    final_recommendations = recommendations.head(5)

    return final_recommendations[['recipe_name', 'ingredients_list', 'Description', 'Procedure', 'Region', 'nutritional_value', 'image_url']].to_dict(orient='records')
